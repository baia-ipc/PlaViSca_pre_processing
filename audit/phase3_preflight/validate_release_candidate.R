...
order_rows <- lapply(names(paths), function(nm) {
  ...
  out <- data.frame(
    ...
    status = if (nrow(x) == 105949L && !anyDuplicated(ids) && identical(ids, rownames(meta)) && identical(ids, canonical)) "PASS" else "FAIL"
  )
  rm(x); gc(); out
})
order_rows[[length(order_rows) + 1L]] <- data.frame(
  ...
  status = if (nrow(meta) == 105949L && !anyDuplicated(rownames(meta)) && identical(rownames(meta), canonical)) "PASS" else "FAIL"
)
...
message("Release candidate validation PASS: canonical order and 7 documented paired-stage missing cells")
