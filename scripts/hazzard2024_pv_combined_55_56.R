...
study_num <- "39223117"
samples <- c(paste0("SRR27021", 955:956))
...
run_id <- rep(paste0("SRR27021", 955:956), length(all_pv))
study_label <- rep("Hazzard2024", length(all_pv))
num_srr <- rep(24, length(all_pv))
pub_year <- rep(2024, length(all_pv))
geographic_location <- rep("Papua New Guinea", length(all_pv))
...
host_id <- c("5309_2", "5309_1")
sample_type <- rep("Mammalian host: blood", length(all_pv))
strain <- rep("Chesson", length(all_pv))
day_post_infection <- c(16, 14)
...
# Save Seurat object
saveRDS(pv.combined.all, file = "hazzard2024_pv_combined_55_56.rds")
# Remove all object
rm(list = ls())
gc()
