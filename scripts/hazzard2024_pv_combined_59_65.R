...
study_num <- "39223117"
samples <- c(paste0("SRR27021", 959:965))
all_pv <- list()
...
run_id <- rep(paste0("SRR27021", 959:965), length(all_pv))
...
sample_type <- list(
    ...
    rep("Vector host: salivary gland", 5)
) %>%
    unlist()
...
day_post_infection <- c(18, 26, 20, 19, 17, 20, 18)
...
# Save Seurat object
saveRDS(pv.combined.all, file = "hazzard2024_pv_combined_59_65.rds")
# Remove all object
rm(list = ls())
gc()
